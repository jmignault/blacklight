...
RSpec.describe Blacklight::IndexFieldPresenter do
  ...
  describe "#value" do
    ...
    context "with a field that links to search" do
      let(:field) { config.index_fields['link_to_search_true'] }

      it "checks for a link_to_search" do
        allow(view_context).to receive(:search_action_path).with('f' => { 'link_to_search_true' => ['x'] }).and_return('/foo')
        allow(view_context).to receive(:link_to).with("x", '/foo').and_return('bar')
        expect(subject).to eq 'bar'
      end
    end

    context "with a named link to search" do
      let(:field) { config.index_fields['link_to_search_named'] }
